...
REFINE_KEYWORDS_CONTENTS = ["优化技术关键词", "保留关键名词", "翻译为地道的英文"]
...
def set_keyword_storage(inputs, storage):
    ...
    for i, repo_info in enumerate(all_repos_info):
        ...
        keywords = (
            ...
            .instruct(
                "仓库{Github仓库名称}的简短描述是{Github仓库简短描述}, 其Readme文件的内容为{Github Readme内容}，请从中提取简短有效的技术关键词, 并按照置信度排序"
            )
            .output([("str",)])
            .start()
        )
...
def filter_keywords(inputs, storage):
    ...
    keywords_filtered = (
        ...
        .instruct(
            "根据之前Github搜索关键词{Github搜索关键词}, 按照{需要过滤的内容}要求过滤不需要的关键词, 然后再按照{需要优化的内容}优化这些关键词, 使Github用户能够快速找到自己感兴趣的技术, 并按照用户可能的感兴趣程度排序"
        )
        .output([("str",)])
        .start()
    )
